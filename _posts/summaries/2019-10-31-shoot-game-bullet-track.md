...
&emsp;&emsp;那么，可行的优化思路是怎样的呢？可以先在服务器预计算出（或者客户端根据当前摄像机朝向自行计算）弹着点，根据预计算的弹着点作为子弹飞行方向，然后在客户端与服务器中同步运行子弹子弹飞行任务，根据子弹运行过程中的碰撞点作为实际弹着点，最后服务器再同步碰撞结果。因此，处理好客户端与服务器的弹道同步，就成为了后续工作的关键。
